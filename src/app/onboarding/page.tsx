...
export default function Onboarding() {
  ...
  async function submit() {
    setLoading(true);
    const siteRes = await fetch('/api/sites', { method:'POST', body: JSON.stringify({ url })});
    const { site_id } = await siteRes.json();
    const goalRes = await fetch('/api/goals', {
      method:'POST',
      body: JSON.stringify({
        site_id, type, page_url: pageUrl, success_mode: successMode,
        thank_you_url: thankYouUrl || null, success_selector: successSelector || null, provider
      })
    });
    const { goal_id } = await goalRes.json();
    await fetch('/api/check-now', { method:'POST', body: JSON.stringify({ goal_id })});
  ...
  return (
    <main className="max-w-xl mx-auto p-6 space-y-4">
      ...
      <input className="border p-2 w-full" placeholder="/contact" value={pageUrl} onChange={e=>setPageUrl(e.target.value)} />

      <label className="block text-sm font-medium mt-4">How do you know it worked?</label>
      <select className="border p-2 w-full" value={successMode} onChange={e=>setSuccessMode(e.target.value as any)}>
      ...
      {successMode === 'thank_you_url' && (
        <>
          ...
          <input className="border p-2 w-full" placeholder="/thank-you" value={thankYouUrl} onChange={e=>setThankYouUrl(e.target.value)} />
        </>
      )}
      {successMode === 'success_selector' && (
        <>
          <label className="block text-sm font-medium mt-2">Success selector</label>
          <input className="border p-2 w-full" placeholder=".elementor-message-success" value={successSelector} onChange={e=>setSuccessSelector(e.target.value)} />
        </>
      )}
      ...
      <button onClick={submit} disabled={loading} className="mt-6 bg-black text-white px-4 py-2 rounded">
